...
/* Secure_prediction */
uint32_t* BNN_Diabetes_Secure_prediction(e_role role, char* address, uint16_t port, seclvl seclvl,
		...
		uint32_t *s2, uint32_t s2nvals, uint32_t *t2, uint32_t t2nvals, uint32_t N) {
	...
	*/
	...
	s_hidden1_1 = Fullconnection(s_W1, s_input, s_fullconnection, s_middlefull, 1704*N, N, 1704, (ArithmeticCircuit*) circ);
	...
	s_hidden2_1 = Fullconnection(s_W2, s_hidden1_3, s_fullconnection1, s_middlefull1, nvals, 3, N, (ArithmeticCircuit*) circ);
	...
	s_out = circ->PutSharedOUTGate(s_hidden2_2);
...
int32_t Enc_circuit(e_role role, char* address, uint16_t port, seclvl seclvl,
		uint32_t nvals, uint32_t bitlen, uint32_t nthreads, e_mt_gen_alg mt_alg,
		e_sharing sharing, uint32_t num, uint32_t *hidden2_2){
	...
	*/
	...
	share *s_hidden2_2, *s_out;

	nvals = 3;

	s_hidden2_2 = circ->PutSharedSIMDINGate(nvals, hidden2_2, bitlen);
	
	s_out = circ->PutOUTGate(s_hidden2_2, ALL);
