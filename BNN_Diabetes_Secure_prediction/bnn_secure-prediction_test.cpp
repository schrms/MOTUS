...
int32_t read_test_options(int32_t* argcp, char*** argvp, e_role* role,
	...
	parsing_ctx options[] =
			{ { (void*) &int_role, T_NUM, "r", "Role: 0/1", true, false }, {
					(void*) nvals, T_NUM, "n",
					"Number of nodes of hidden layer, default: 10", false, false }, {
...
int main(int argc, char** argv) {
	...
	e_role role;
	uint32_t bitlen = 32, nvals = 10, secparam = 128, nthreads = 1;
	uint16_t port = 7766;
	...
	uint32_t N = nvals;	//Number of nodes of hidden layer, default: 10
	...
	uint32_t *input;
	input = new uint32_t[1704];
	uint32_t input_flen;
	uint32_t input_col = 0;
	uint32_t input_row = 0;
	string input_fname = "./param/input[0].csv";
	input_flen = fileread(input, input_fname, &input_col, &input_row, 1704); 
	...
	uint32_t W2_flen = 3*N; //length of W3
	uint32_t W2_col = 3; //col of matrix
	...
	s2_flen = fileread(s2, s2_fname, &s2_col, &s2_row, 3); 
	...
	t2_flen = fileread(t2, t2_fname, &t2_col, &t2_row, 3); 
	...
	hidden2_2 = BNN_Diabetes_Secure_prediction(role, (char*) address.c_str(), port, seclvl, 1, 32,
			...
			out_s2, out_s2nvals, out_t2, out_t2nvals, N);

	
	/* decrypt hidden2_2 and convert decrypted hidden3_2 into output.csv */

	Enc_circuit(role, (char*) address.c_str(), port, seclvl, 1, 32,
			nthreads, mt_alg, S_ARITH, nvals, hidden2_2);
