...
int main(int argc, char** argv) {
	...
	W1posids_index = new uint32_t[16];
	int index_W1 = 0;
	int size_W1 = 0;
	...
	uint32_t *u1;
	u1 = new uint32_t[24*24*16];

	uint32_t *u1_temp;
	u1_temp = new uint32_t[16];
	uint32_t u1_flen;
	uint32_t u1_col = 0;
	uint32_t u1_row = 0;
	string u1_fname = "./param/u1.csv";
	u1_flen = fileread(u1_temp, u1_fname, &u1_col, &u1_row, 16); 
	...
	if(role == SERVER) { 
		s_u1 = circ->PutSIMDINGate(24*24*16, u1, bitlen, SERVER);
	} else { //role == CLIENT
		s_u1 = circ->PutDummySIMDINGate(24*24*16, bitlen);
	}
	...
	W2posids_index = new uint32_t[16];
	int index_W2 = 0;
	int size_W2 = 0;
	...
	uint32_t *u2;
	u2 = new uint32_t[8*8*16];

	uint32_t *u2_temp;
	u2_temp = new uint32_t[16];
	uint32_t u2_flen;
	uint32_t u2_col = 0;
	uint32_t u2_row = 0;
	string u2_fname = "./param/u2.csv";
	u2_flen = fileread(u2_temp, u2_fname, &u2_col, &u2_row, 16); 
	...
	if(role == SERVER) { 
		s_u2 = circ->PutSIMDINGate(8*8*16, u2, bitlen, SERVER);
	} else { //role == CLIENT
		s_u2 = circ->PutDummySIMDINGate(8*8*16, bitlen);
	}
	...
	W3posids_index = new uint32_t[100];
	int index_W3 = 0;
	int size_W3 = 0;
	...
	u3_flen = fileread(u3, u3_fname, &u3_col, &u3_row, 100); 
	...
	uint32_t *W4_temp;
	W4_temp = new uint32_t[W4_num];

	uint32_t *W4posids_index;
	W4posids_index = new uint32_t[10];
	int index_W4 = 0;
	int size_W4 = 0;
	...
	for(int i=0; i<10; i++){
		...
		W4posids_index[i] = index_W4;
		index_W4 = 0;
	}

	uint32_t *posids_W4;
	posids_W4 = new uint32_t[size_W4];

	for(int i=0; i<size_W4; i++){
		posids_W4[i] =W4_temp[i];
	}
	...
	hidden3_2 = BNN_Mnist_Secure_prediction(role, (char*) address.c_str(), port, seclvl, 1, 32,
			...
			posids_W1, posids_W2, posids_W3, posids_W4, 
			W1posids_index, W2posids_index, W3posids_index, W4posids_index);
